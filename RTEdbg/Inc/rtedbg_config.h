 ...
 *
 * @note This configuration file was made for the RTEcomLib demo.
...
/* Define the processor core and specific hardware included files for a specific project.
 * Include only one of the following items in your final project and adjust it accordingly
 * to your CPU or timer clock frequency. A few examples have been prepared for some popular
 * ARM Cortex M cores. Enable just one of them.
 *
 * Note: This file is the same for all demo projects and contains a conditional compilation
 *       to fit into all of them with as little modification as possible. Enable only the
 *       timestamp drivers that are appropriate for the particular demo (exist in the folder).
 */

// Example: Timer SYSTICK used as the timestamp counter
#define RTE_TIMER_DRIVER  "rtedbg_timer_systick.h"
#define RTE_TIMESTAMP_SHIFT   3U    // Divide the timestamp by 2^3 = 8
...
#define RTE_CPU_DRIVER  "rtedbg_generic_irq_disable.h" // Reserving buffer space by temporarily disabling interrupts
...
#define RTE_MINIMIZED_CODE_SIZE           2
